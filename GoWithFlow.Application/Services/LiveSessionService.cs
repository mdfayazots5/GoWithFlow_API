...
public sealed class LiveSessionService : ILiveSessionService
{
	...
	public async Task<ApiResponse<TurnStateResponseDto>> GetCurrentTurnAsync(long sessionId, CancellationToken cancellationToken = default)
	{
		...
		var currentTurn = await EnsureCurrentTurnAsync(sessionId, "System", cancellationToken);

		if (currentTurn is null)
		{
			return ApiResponse<TurnStateResponseDto>.FailureResult(new[] { "Active turn was not found for this session." }, "Current turn not found.");
		}

		return ApiResponse<TurnStateResponseDto>.SuccessResult(currentTurn, "Current turn retrieved successfully.");
	}
	...
	public async Task<ApiResponse<TurnStateResponseDto>> ShiftTurnAsync(TurnShiftRequestDto dto, long userId, CancellationToken cancellationToken = default)
	{
		...
		var nextTurnResponse = await CreateNextTurnAsync(session, currentTurnEntity.TurnIndex + 1, user.FullName, cancellationToken);

		if (nextTurnResponse is null)
		{
			return ApiResponse<TurnStateResponseDto>.FailureResult(new[] { "No further turns remain in this session. Complete the session." }, "Turn shift completed.");
		}

		return ApiResponse<TurnStateResponseDto>.SuccessResult(nextTurnResponse, "Turn shifted successfully.");
	}
	...
	private async Task<TurnStateResponseDto?> EnsureCurrentTurnAsync(long sessionId, string requestedBy, CancellationToken cancellationToken)
	{
		...
		if (session is null || string.Equals(session.Status, SessionStatusType.ACTIVE.ToString(), StringComparison.OrdinalIgnoreCase) == false)
		{
			return null;
		}
	...
	private async Task<TurnStateResponseDto?> CreateNextTurnAsync(Session session, int nextTurnIndex, string createdBy, CancellationToken cancellationToken)
	{
		...
		if (activeMembers.Count == 0 || orderedUtterances.Count == 0 || nextTurnIndex > orderedUtterances.Count)
		{
			return null;
		}
		...
		var activeMember = activeMembers.FirstOrDefault(sessionMember =>
			string.Equals(sessionMember.SlotName, nextUtterance.SpeakerLabel, StringComparison.OrdinalIgnoreCase));
		...
		return await _liveSessionRepository.GetCurrentTurnAsync(session.SessionId, cancellationToken);
	}
