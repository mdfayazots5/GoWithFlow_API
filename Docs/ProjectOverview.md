...
- Business columns:
  ...
  - `ShowHardWords BIT NOT NULL DEFAULT 0` *(2026-06-18 — Q&A "Show Hard Words"; Migration 39, applied to Supabase. **DEPRECATED same day** — gating moved to the client-side `SessionPreferencesService.showHardWords` room pref; column still written by `uspSetSessionAiConfig` but no longer read. Safe to drop later.)*
  - `ShowHardWordsInAnswer BIT NOT NULL DEFAULT 0` *(2026-06-18 — Q&A; Migration 41 `41_add_hard_words_in_answer.sql`, applied to Supabase. **DEPRECATED same day** — replaced by the client room pref; column still written but unused. Safe to drop later.)*
...
Body (CreateSessionRequestDto):
  ...
  - ShowHardWords / ShowHardWordsInAnswer (bool, optional): **DEPRECATED 2026-06-18 — no longer sent by the client.** Q&A "Show Hard Words" is now a client-side room-settings preference (`SessionPreferencesService.showHardWords`), not a create-session/DB flag. The DTO fields + clamp still exist (default false) but are unused; see the "Question & Answer Hard Words" flow.
```
...
ApiResponse<TurnStateResponseDto>
  ...
  - ShowHardWords / ShowHardWordsInAnswer (bool): **DEPRECATED 2026-06-18 — still projected from `tblSession` but no longer read by the client** (gating moved to the `SessionPreferencesService.showHardWords` room pref).
  - HardWords (HardWordDto[] — `{word, meaning?}`): *(2026-06-18 — Q&A)* "Key words". The backend ALWAYS populates this on Q&A turns (gated on `HideScriptText`, not a flag), capped to 5: **`IsFacilitatorTurn`** → this interviewer utterance's words; **answer turn** → the words of the most recent preceding utterance on the same script (the question being answered); non-Q&A → empty. When empty the raw `Utterance.HardWords` is nulled. Frontend gates DISPLAY on the room pref `showHardWords` (default false): `listener-screen` renders the question-turn list at the bottom; on the answer turn `session-room` renders a room-level panel from the `answerHardWords` signal below `<app-speaker-screen>` (decoupled from `SpeakerScreen` to protect the recording auto-start). The optimistic `TURN_SHIFT` patch resets both `hardWords: []` and `answerHardWords` to avoid a stale flash.
```
...
**Purpose:** Q&A practice aid. Each Q&A pair can carry hard/important words on the Interviewer row. When the in-room **"Show Hard Words"** preference is on, the candidate sees a **"Key words"** panel on BOTH the question turn (while the AI interviewer reads the question) AND their own answer turn (so they recall/use the words while speaking). The words shown on the answer turn are the *question's* words (carried forward). **Q&A only.**

**Control = client-side room preference (NOT a server/DB flag) — changed 2026-06-18:** a single toggle **"Show Hard Words"** lives in the live-session **room settings sheet** (gear icon), stored in `SessionPreferencesService` (`gwf_session_prefs.showHardWords`, **default false** = blind by default), alongside `defaultVoiceStarter`/`autoSubmitOnStop`/`listenVoiceBroadcast`. It is per-user, live (instant show/hide), and needs no endpoint or SignalR. The toggle is only displayed in Q&A sessions (gated on `turnState.hideScriptText`). The old create-session toggles were **removed**.

**Entry Points:** Excel upload (admin) authors Column I; the toggle lives in the room settings sheet; the words render in `live-session` on both the question and answer turns.

**UI Trigger:** room settings sheet "Show Hard Words" switch (Q&A only). When on, the panel renders on the interviewer/listen turn and on the candidate's answer turn.
...
**Turn payload (read) — backend ALWAYS sends the words on Q&A turns; the client decides display:**
- `GetCurrentTurnAsync` projects the raw `utterance.HardWords` → `UtteranceResponseDto.HardWords`. After materialization the `HardWords` list (`HardWordDto{Word, Meaning?}`, capped to 5) is populated by category (gated on `HideScriptText`, the Q&A marker — no DB flag involved):
  - **`HideScriptText && IsFacilitatorTurn`** (Q&A question turn) → parse THIS utterance's `HardWords`.
  - **`HideScriptText && !IsFacilitatorTurn`** (Q&A answer turn) → the answer row has no words of its own, so it pulls them from the most recent **preceding utterance on the same `ScriptId` with non-null `HardWords`** (`SequenceId < current`, `ORDER BY SequenceId DESC`, first) — i.e. the question being answered. One extra indexed lookup, Q&A only.
  - else (non-Q&A) → empty (skips the lookup).
  When the list is empty it also nulls the raw `Utterance.HardWords` so a half-populated field is never sent.
- **Deprecated/vestigial:** `tblSession.ShowHardWords` / `ShowHardWordsInAnswer` columns + `TurnStateResponseDto.ShowHardWords*` fields still exist and are still written by `uspSetSessionAiConfig`, but are **no longer read for gating** (the client pref replaced them). Left in place to avoid a DB revert; safe to drop in a future cleanup.

**Frontend render (both panels at the BOTTOM, gated on `sessionPrefs.prefs.showHardWords`):**
- `listener-screen.component` (question turn): renders `turnState.hardWords` as a wrapped chip list ("Key words to remember") below the utterance, gated on the pref. Injects `SessionPreferencesService`.
- `session-room.component` (answer turn): the candidate is the **speaker**, so the words render in a **room-level** panel ("Key words to use") **below `<app-speaker-screen>`**, driven by the `answerHardWords` signal and gated on the pref — NOT passed into `SpeakerScreenComponent`'s `turnState` input. This is deliberate: `SpeakerScreen.ngOnChanges` unconditionally cancels the auto-start/auto-submit timers, so re-feeding canonical state into it on the speaker's own turn would kill the pending mic auto-start. `updateState` sets `answerHardWords = amSpeaker ? state.hardWords : []`; `handleTurnShift` resets it to `[]` (no stale flash). Same chip styling, ≤14px body, wraps, never scrolls sideways.
...
- Positional contracts that must stay in lockstep: Excel cell 9 ↔ `tblUtterance.HardWords`; C# TVP/JSONB column order ↔ `uspBulkInsertUtterance`; C# param order ↔ PG positional `uspsetsessionaiconfig` (`@ShowHardWords` 7th, `@ShowHardWordsInAnswer` 8th — Migration 41 recreated the 10-arg signature; the params are written but no longer read for gating).
