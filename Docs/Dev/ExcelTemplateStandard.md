...
### 10.0 How to Use This Protocol (Updated — DB-Connected Workflow)

The Admin Script Upload Wizard provides a **Copy Prompt** button for each category.
This button generates a prompt that is automatically enriched with live data from the GoWithFlow database:

| What the prompt includes | Where it comes from |
|---|---|
| Category speaker label rules | Static (this document §6) |
| Min/max row count | Static (§8.3) |
| Mandatory column rules | Static (§8.3) |
| GrammarTag values already in use | Live DB — `tblScript.GrammarFocusTag` for this category |
| ContextTag values already in use | Live DB — `tblScript.ContextTag` for this category |
| Full approved GrammarTag list | Static (§9.3) |
| Active script count for the category | Live DB count |

**Admin Workflow:**
1. In the Upload Wizard (Step 1), click the category button for the script type you want to create.
2. The platform downloads the category-specific `.xlsx` template AND shows the pre-built Claude prompt.
3. The prompt already contains the actual grammar tags and context tags in use in this platform — so Claude produces content that is consistent with what already exists.
4. Click **Copy Prompt** → paste into claude.ai → receive JSON output.
5. Use the JSON to create your Excel file and upload it via the wizard.

**DB Prompt Storage:**
Prompts are stored in `tblscriptprompttemplate` (one row per category, seeded from this document).
Each row records: `category`, `prompttext` (full prompt), `sourceref` (ExcelTemplateStandard.md §section), `version`, `isactive`.
Admins can update any prompt directly in the DB without code changes.

**DB Prompt Data Endpoint:**
`GET /api/v1/scripts/prompt-data?category={category}` — fetches the stored prompt from `tblscriptprompttemplate` and appends live DB enrichment (grammar tags in use, context tags in use, active script count).
Returns: `fullPrompt` (complete ready-to-copy string), `grammarTagsInUse`, `contextTagsInUse`, `approvedGrammarTags`, `activeScriptCount`.

**Prompt data flow:**
```
tblscriptprompttemplate.prompttext   ← seeded from ExcelTemplateStandard.md §6.x
         +
tblscript.grammarfocustag (live)     ← actual tags in use for this category
tblscript.contexttag (live)          ← actual contexts in use for this category
         ↓
GET /api/v1/scripts/prompt-data → fullPrompt (returned to admin Upload Wizard)
         ↓
Admin copies → pastes into claude.ai → receives JSON → uploads via wizard
```

**Sample Template Data Source:**
`GET /api/v1/scripts/sample-template?category={category}` — when active scripts exist for the category, the first 4 utterances of the most recently uploaded script are used as sample rows. Falls back to static sample rows only when no scripts exist.

---

### 10.1 AI Prompt Template (Output Contract)

The prompt (generated by the wizard and copied by the admin) instructs Claude to return strict JSON with the following contract:

```json
{
  "metadata": {
    "scriptTitle": "[descriptive title]",
    "category": "[exact category value — e.g. Grammar Drill]",
    "grammarFocusTag": "[from the approved list or in-use tags]",
    "contextTag": "[from existing context tags or a new realistic one]",
    "complexityLevel": 3,
    "targetAgeGroup": "Adult",
    "hintLanguage": "Telugu"
  },
  "rows": [
    {
      "sequenceId": 1,
      "speakerLabel": "[category-required label]",
      "englishText": "[turn content — grammatically correct, under 512 chars]",
      "hintText": "[Telugu translation — required if category needs column D]",
...
**Rules enforced by the prompt (per category):**
- Speaker labels: per §6 — category-specific labels only
- Row count: within category min/max per §8.3
- Speaker alternation: no speaker has 3+ consecutive turns
- SequenceId: starts at 1, increments by 1, no gaps
- FocusWord must appear verbatim in the same row's EnglishText
- Output is strict JSON — no markdown, no code fences, no trailing commas
...
- [ ] GrammarTag: consistent within category constraint; from approved list or in-use tags from DB
...
- [ ] Content is distinct from existing scripts in the platform (check activeScriptCount in prompt data)
