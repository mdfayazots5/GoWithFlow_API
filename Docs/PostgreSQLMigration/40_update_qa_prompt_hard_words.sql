-- Migration 40: Update the "Question & Answer" prompt template to generate the Hard Words aid (2026-06-18).
-- Adds Column I (HardWords) generation rules + JSON field so AI-generated Q&A sheets carry 2-4
-- 'word:meaning' pairs on Interviewer rows. Pairs with the "Show Hard Words" session toggle (migration 39).
-- SQL Server equivalent: Backend/Docs/SqlServerSeed/40_update_qa_prompt_hard_words.sql. Idempotent (UPDATE).

UPDATE public.tblscriptprompttemplate
SET sourceref = 'ExcelTemplateStandard.md §6.7, §7.1, §9, §10',
    prompttext = $PROMPT$
...
$PROMPT$
WHERE category = 'Question & Answer';
