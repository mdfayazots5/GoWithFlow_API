-- Migration 37: Seed the "Question & Answer" prompt into tblscriptprompttemplate (PostgreSQL).
-- Phase 1.b of the Question & Answer category (see Backend/Docs/QuestionAnswerCategoryPlan.md).
-- SQL Server equivalent: Backend/Docs/SqlServerSeed/37_add_question_answer_prompt_template.sql
--
-- Without this row, GetPromptDataForCategoryAsync returns the "[Prompt template not found]" base and the
-- admin upload wizard falls back to the locally-built prompt — uploads still work, but the admin's
-- copy-ready Claude prompt is not DB-sourced. This seed makes the DB the source of truth, matching the
-- other six categories seeded in migration 19.
--
-- Note: the retired "Repractice Round" row from migration 19 is intentionally LEFT in place — it is
-- harmless (the category is simply no longer offered in the upload UI) and keeps legacy data readable.
...
$PROMPT$, 'ExcelTemplateStandard.md §6.7, §7.1, §9, §10')
