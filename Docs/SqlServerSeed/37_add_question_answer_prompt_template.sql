-- SQL Server seed: "Question & Answer" prompt for dbo.tblScriptPromptTemplate.
-- Phase 1.b of the Question & Answer category (see Backend/Docs/QuestionAnswerCategoryPlan.md).
-- PostgreSQL equivalent: Backend/Docs/PostgreSQLMigration/37_add_question_answer_prompt_template.sql
--
-- The SQL Server table is created by the EF migration 20260601000003_AddScriptPromptTemplate_Phase10,
-- which (per its own note) does NOT seed rows — category prompts are applied via separate data scripts
-- like this one. Idempotent: re-running is a no-op (UK_tblScriptPromptTemplate_Category + the guard).
-- PromptText is identical to the PostgreSQL migration; the only difference is doubled apostrophes
-- ('') as required by SQL Server string literals.

IF NOT EXISTS (SELECT 1 FROM dbo.tblScriptPromptTemplate WHERE Category = N'Question & Answer')
BEGIN
    INSERT INTO dbo.tblScriptPromptTemplate (Category, PromptText, SourceRef)
    VALUES (N'Question & Answer', N'
